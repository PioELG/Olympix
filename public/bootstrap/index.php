...
//Question sur php info
 ...
 //Préparation du mot de passe de l’utilisateur 
...
$requete = "UPDATE T_Compte_COM SET COM_motDePasse ='".$password."' WHERE
 COM_idCompte = (SELECT MAX(COM_idCompte) FROM T_Compte_COM);";
...
// Recupération du document inséré




$requete2= "SELECT DOC_nomDocument FROM T_Document_DOC  WHERE DOC_idDocument = (SELECT MAX(DOC_idDocument) FROM T_Document_DOC)";
 ...
 else
{
 ...
 while ($row = $resultat2->fetch_assoc()) {
    $nomDocument = $row["DOC_nomDocument"];
    }
    $cheminrelatif = "https://obiwan.univ-brest.fr/~e22406560/gabarit/front_office/documents/";
    $cheminAbsolu =$cheminrelatif.$nomDocument;
...
<a href="<?php echo $cheminAbsolu; ?>" target="_blank">
      Lien vers l'image qui a été téléchargé
    </a>
